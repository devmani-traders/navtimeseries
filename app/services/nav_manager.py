...
class NavManager:
    def __init__(self):
        ...
        if not config.USE_GCS and not os.path.exists(self.output_folder):
    ...
    def download_nav_all(self):
        ...
        for attempt in range(3):
            try:
                ...
                storage.write_text(response.text, self.nav_all_file)
    ...
    def update_master_list_with_codes(self, master_list_path):
        ...
        try:
            df = storage.read_csv(master_list_path)
        except Exception as e:
        ...
        # Save back
        storage.write_csv(df, master_list_path)
    ...
    def _load_existing_data(self, filepath):
        ...
        if storage.exists(filepath):
            try:
                existing_df = storage.read_csv(filepath)
    ...
    def _save_data(self, df, filepath, scheme_name):
        ...
        # Save
        storage.write_csv(final_df, filepath)
